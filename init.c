...
/*   Updated: 2020/05/08 15:06:40 by arz              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	z_min_max(t_fdf *data)
{
	...
	while (i < data->map->y_size)
	{
		...
		{
			if (data->map->matrix_map[i][j] <= INT_MAX && data->map->matrix_map[i][j] > data->max_z)
				data->max_z = data->map->matrix_map[i][j];
			if (data->map->matrix_map[i][j] >= INT_MIN && data->map->matrix_map[i][j] < data->min_z)
				data->min_z = data->map->matrix_map[i][j];
			j++;
		}
		i++;
	}
	data->med_z = (data->min_z + data->max_z) / 2;
}
